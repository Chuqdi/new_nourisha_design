...
export default function Button({
  ...
  return (
    <button
      className={`
        h-8 py-4 px-[1.25rem] min-w-[10rem]
        text-sm font-inter hover:opacity-80  rounded-full flex items-center justify-center shadow-btn gap-2 font-bold
        ${
          variant === "primary"
            ? "bg-primary-orange-900 border-none text-white"
            : "bg-white border-[1px] border-primary-orange-900 text-primary-orange-900"
        }
        ${fullWidth && "w-full"}
    ...
    >
      ...
      {isRightIconed && <img src="/images/icons/right.svg" />}
      {iconName && (
